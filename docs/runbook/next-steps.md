...
### 1.6 企业微信群机器人（×2：美股 + A 股）

**用途**：备用推送通道

1. 企业微信 → 创建 2 个群（如"News-US"、"News-CN"）
2. 群设置 → 群机器人 → 添加机器人 → 选"自定义"
3. 复制 webhook URL（`https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=xxx`）
4. 填进 `secrets.yml`：
   ```yaml
   push:
     wecom_hook_us: https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=xxx
     wecom_hook_cn: https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=yyy
   ```

**坑**：
- 企业微信和微信不一样，需要先有企业（个人也可以注册"企业"）
- 群机器人推送格式有限制（无按钮、Markdown 受限）

### 1.7 阿里云 OSS（图表图片托管）

**用途**：K 线、财报图等存这里，飞书/TG 通过 URL 引用
...
2. 配置：
   ...
   - **地域**：选你最近的（如杭州）
   - **存储类型**：标准存储
   - **读写权限**：**公共读**（图表 URL 要能匿名访问）
   - **存储冗余**：本地冗余（最便宜）
3. 创建后进入 Bucket → 权限管理 → Bucket 授权策略 → 设置「文件公共读」（如果创建时没选）
4. **生命周期管理** → 添加规则：30 天后删除（避免无限堆积）
5. 拿 AccessKey：右上角头像 → AccessKey 管理 → 创建 RAM 用户 → 授权 `AliyunOSSFullAccess`（仅这个 bucket 更安全）
6. 填进 `secrets.yml`：
   ```yaml
   oss:
     endpoint: oss-cn-hangzhou.aliyuncs.com   # 按实际地域
     bucket: news-charts-qingbin
     access_key_id: LTAI...
...
**坑**：
- OSS 月成本：< ¥1（个人项目流量极小）
- 公共读 = 任何人有 URL 都能看图，不要存敏感图

### 1.8 Bark（iOS 系统级告警通道）
...
2. F12 打开开发者工具 → Application → Cookies → `https://xueqiu.com`
3. 复制所有 cookie，组装成 `name1=value1; name2=value2; ...` 格式
4. 关键 cookie：`xq_a_token`、`xqat`、`xq_r_token`、`xq_id_token`、`u`、`device_id`
5. 填进 `secrets.yml`：
   ```yaml
   sources:
     xueqiu_cookie: "xq_a_token=xxx; xqat=xxx; ..."
   ```

#### 同花顺 Cookie

类似流程，登录 [10jqka.com.cn](https://10jqka.com.cn)，F12 复制 cookie。

```yaml
sources:
  ths_cookie: "uid=xxx; v=xxx; ..."
```

**坑**：
- Cookie 通常 7-30 天过期，过期后该源会反爬告警，需要重新拿
- **不要 commit cookie 到 git**（`secrets.yml` 已 gitignored）
- 雪球和同花顺有反爬，请求频率不要超 spec 设定（雪球 5min 一次）

### 1.11 Finnhub（美股新闻 API，可选但强烈推荐）
...
push:
  ...
  feishu_sign_cn: 真实值
  feishu_encrypt_key: 32位随机字符串    # v0.1.1 新加，飞书 webhook 验签
  wecom_hook_us: https://...
  wecom_hook_cn: https://...
storage:
...
oss:
  ...
  bucket: news-charts-qingbin
  access_key_id: LTAI真实值
  access_key_secret: 真实值
sources:
  finnhub_token: 真实值
  tushare_token: 真实值
  xueqiu_cookie: "..."
...
期望看到：`alembic_version audit_log chart_cache daily_metrics dead_letter digest_buffer entities news_entities news_fts news_processed push_log raw_news relations source_state`
...
- [ ] 1.6 创建 2 个企业微信群机器人 webhook
- [ ] 1.7 阿里云 OSS bucket（公共读 + 30 天生命周期）+ AccessKey
- [ ] 1.8 iOS 装 Bark + 拿 server URL
- [ ] 1.9 tushare 注册 + token
- [ ] 1.10 雪球 + 同花顺 cookie
- [ ] 1.11 Finnhub 注册（可选）
