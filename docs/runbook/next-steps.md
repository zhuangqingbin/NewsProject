...
`docker compose up -d` 会同时启动 `news_pipeline` + `datasette` 两个容器。
默认 datasette 绑在 `127.0.0.1:8001`（仅本机访问，安全）。
...
**远程访问服务器上的 datasette（推荐 SSH 隧道）**：
```bash
ssh -L 8001:localhost:8001 user@<服务器IP>
# 然后本机浏览器开 http://localhost:8001
```
...
- `raw_news` / `news_processed` / `push_log` → 流程数据
- `entities` / `news_entities` / `relations` → LLM 抽出的实体网络（图谱原料）
- `news_fts` → SQL 全文搜索（`SELECT * FROM news_fts WHERE news_fts MATCH 'NVDA'`）
- `daily_metrics` → 抓取/推送/成本指标
- `dead_letter` → 失败任务清单

#### 公网暴露（高危）

如果非要把 8001 暴露公网（不推荐）：
1. 改 `compose.yml` 把 `"127.0.0.1:8001:8001"` 改成 `"8001:8001"`
2. **必须**加 nginx 反代 + basic auth（datasette 没内置认证）
3. 防火墙只放行你的家庭 IP
...
5. **I10**: 雪球/同花顺反爬识别加上「200 + 0 items + HTML」检测
